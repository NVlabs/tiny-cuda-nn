...
class SDF(nn.Module):
	def __init__(self, hash=True, n_levels=12, log2_hashmap_size=15, base_resolution=16, smoothstep=False) -> None:
		...
		print("params_enc.length: ", len(params_enc), params_enc[0:64])
	...
	def saveMesh(self, gridres, file, it, level=0):
		...
		print("saveMesh - grid.shape:", grid.shape)
		print("saveMesh - res:", res)
		grid_size = 64
		sdf = []
		spacing = 64*64*64
	...
	def forward_with_nablas(self, x):
		with torch.enable_grad():
			...
			print("within forward_with_nablas: x.shape: ", x.shape)
			sdf = self.forward(x)
...
class SDF_TCNN(nn.Module):
	def __init__(self, hash=True, n_levels=12, log2_hashmap_size=15, base_resolution=16, smoothstep=False) -> None:
		...
		file_dir = os.getcwd()
		# load encoder params but initialization params are the same no need to assign
		...
		num_enc_params = 12196240 # 12196240
...
if __name__ == '__main__':
	...
	def print_TCNN_SDF(model):
		...
		dec_grad_3 = model.encoder_decoder.params.grad[10240:10304] # 3rd hidden layer
	...
	def test_train():
		...
		with tqdm(range(10000)) as pbar:
			for _ in pbar:
				...
				# leverage 2 sets of data for numeric alignment
				#r = torch.tensor([[-0.4805,  0.2734,  0.3652], [ 0.3096,  0.1665, -0.1348]], device='cuda', dtype=torch.float16)
				r = torch.tensor([[-0.4805,  0.2734,  0.3652]], device='cuda', dtype=torch.float16)
				
				sdf_, nablas = model.forward_with_nablas(r)
				eikonal_loss = torch.sum(torch.abs(torch.norm(nablas, p=2, dim=1) - 1))
				...
				# print grad details when using pytorch SDF in line 528
				#print_torch_SDF(model)

				# print grad details when using TCNN SDF in line 524
